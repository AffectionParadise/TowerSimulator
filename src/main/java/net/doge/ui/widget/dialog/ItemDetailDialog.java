...
import net.doge.model.ItemProbabilityModel;
...
public class ItemDetailDialog extends GDialog<ItemProbabilityModel<Item>> {
    ...
    public void init() {
        ...
        if (item.isBox()) {
            ...
            double weightSum = subItemSampler.getWeightSum();
            subItemSampler.getModels().forEach(model -> listModel.addElement(new ItemProbabilityModel<>(model.getItem(), model.getWeight() / weightSum)));
            // 多爆盒子物品
            if (item.isPackedBox()) {
                item.getExtraSubItemSamplers().forEach(sampler -> {
                    double ws = sampler.getWeightSum();
                    sampler.getModels().forEach(model -> {
                        if (!model.isEmpty())
                            listModel.addElement(new ItemProbabilityModel<>(model.getItem(), model.getWeight() / ws));
                    });
                });
        ...
        list.addMouseListener(new MouseAdapter() {
            ...
            public void mouseReleased(MouseEvent e) {
                ItemProbabilityModel<Item> model = list.getSelectedValue();
